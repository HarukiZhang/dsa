...
int main() {
    ...
    for (int index = n + 1; index <= n + m; index++) {// for each gas loc;
        double mindis = inf, aver = 0;
        fill(dis, dis + 1020, inf);
        fill(visit, visit + 1020, false);

        dis[index] = 0;
        for (int i = 0; i < n + m; i++) {//for all loc;
            int u = -1, minn = inf;
            //find crt min vertex;
            for (int j = 1; j <= n + m; j++) {//for all loc;
            ...
            visit[u] = true;
            //update dist;
            for (int v = 1; v <= n + m; v++) {// for each direct edge of crt vertex;
