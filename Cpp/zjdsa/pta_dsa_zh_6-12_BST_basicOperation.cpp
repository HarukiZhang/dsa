...
BinTree Delete( BinTree BST, ElementType X ){
    ...
    while (toDel){
        if (X > toDel->Data){
            if (toDel->Right) {
                parent = toDel; toDel = toDel->Right;
            }
            else toDel = NULL;
        }
        else if (X < toDel->Data){
            if (toDel->Left){
                parent = toDel; toDel = toDel->Left;
            }
            else toDel = NULL;
        }
    ...
    if (toDel){
        ...
        /*Situ2: degree 2 node*/
            ...
            while (maxN->Right) {
                maxPrt = maxN; maxN = maxN->Right;
            }
            /*Subtitute toDel with the value of maxN;*/
            toDel->Data = maxN->Data;
            /*Then the problem has been converted to delete maxN;*/
            Position descendant = NULL;
            if (maxN->Left) descendant = maxN->Left;
            if (maxPrt->Left == maxN) maxPrt->Left = descendant;
            else maxPrt->Right = descendant;
            delete maxN;
        }
    }
